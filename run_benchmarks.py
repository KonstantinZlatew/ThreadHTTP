...
from benchmark_lib import WrkBenchmark, save_results
...
def main():
    ...
    # Инициализация
    # wrk = WrkBenchmark(wrk_path="/usr/local/bin/wrk")
    wrk = WrkBenchmark()
    
    # Проверка за wrk
    print("\n1. Checking prerequisites...")
    if not wrk.check_wrk_installed():
        print("✗ ERROR: wrk is not installed or not in PATH")
        print("\nTo install wrk:")
        print("  Ubuntu/Debian: sudo apt-get install wrk")
        print("  macOS: brew install wrk")
        print("  From source: https://github.com/wg/wrk")
        sys.exit(1)
    ...
    print(f"  Payload sizes: {[wrk._format_size(s) for s in PAYLOAD_SIZES]}")
    ...
    results_21_22 = wrk.run_concurrency_test(
    ...
    results_23 = wrk.run_throughput_test(
        payload_sizes=PAYLOAD_SIZES,
        connections=10,  # Константна паралелност
        duration=DURATION
    )
    ...
    print(f"{'Connections':<15} {'Req/s':<12} {'Latency (ms)':<15} {'Failed':<10}")
    print("-" * 52)
    for r in results_21_22:
        print(f"{r['connections']:<15} "
